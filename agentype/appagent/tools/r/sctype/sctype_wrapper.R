...
#' db_=sctype_source()
#' 
sctype_source <- function(script_dir = NULL){
    # If directory is provided, use it directly
    if (is.null(script_dir)) {
        # Auto-detect directory (for backward compatibility)
        script_dir <- tryCatch({
    ...
    # load tissue auto detect (from local file)
    source(file.path(script_dir, "auto_detect_tissue_type.R"))
    # load gene set preparation function (from local file)
    source(file.path(script_dir, "gene_sets_prepare.R"))
    # load cell type annotation function (from local file)
    source(file.path(script_dir, "sctype_score_.R"))
    # load ScType database (from local file)
    db_ = file.path(script_dir, "ScTypeDB_full.xlsx")
    return(db_)
}
...
run_sctype <- function(seurat_object, known_tissue_type = NULL, assay = "RNA", scaled = TRUE, custom_marker_file = NULL, plot = FALSE, name = "sctype_classification", sctype_dir = NULL) {
    db_=sctype_source(sctype_dir)
    ...
    package_type <- ! "layers" %in% names(attributes(seurat_object[[assay]]))
