#!/usr/bin/env python3
"""
agentype - SubAgent Prompt模板(兼容层)
重定向到新的统一Prompt管理系统
...
def get_system_prompt_template(language: str) -> str:
    ...
    return manager.get_system_prompt('subagent')


def get_fallback_prompt_template(language: str) -> str:
    """获取Fallback Prompt模板"""
    manager = get_prompt_manager(language)
    return manager.get_fallback_prompt('subagent')


def get_user_query_templates(language: str) -> Dict[str, str]:
    """获取用户查询模板"""
    manager = get_prompt_manager(language)
    return manager.get_user_query_templates('subagent')


def get_correction_template(language: str) -> str:
    """获取修正模板"""
    manager = get_prompt_manager(language)
    return manager.get_correction_template('subagent')
