...
class GlobalTimestampManager:
    ...
    统一管理数据处理流程中的时间戳，确保同一批次处理的所有文件使用相同的时间戳
    """
    ...
    def generate_new_timestamp(cls) -> str:
        ...
        Returns:
            新生成的时间戳，格式为YYYYMMDD_HHMMSS
        """
        from datetime import datetime
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        cls._current_timestamp = timestamp
        logger.info(f"生成新时间戳: {timestamp}")
        return timestamp
    ...
    @classmethod
    def set_timestamp(cls, timestamp: str):
        """设置指定的时间戳
        ...
        """
        cls._current_timestamp = timestamp
        logger.info(f"设置时间戳: {timestamp}")
    
    @classmethod
    def clear_timestamp(cls):
        """清除当前时间戳"""
        cls._current_timestamp = None
        logger.info("清除当前时间戳")
    
    @classmethod
    def get_timestamped_filename(cls, base_filename: str, extension: str = None) -> str:
        """获取带时间戳的文件名
